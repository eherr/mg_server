...
from anim_utils.motion_editing.motion_grounding import MotionGrounding
...
class StateMachineController(ComponentBase, MotionStateInterface):
...
class MorphableGraphStateMachine(StateMachineController):
    ...
    def set_visualization(self, visualization):
        ...
        #self._visualization = SkeletonVisualization(self.scene_object, self.color)
        #self._visualization.set_skeleton(self._graph.skeleton)
        #self.line_renderer = DebugLineRenderer([0,0,0], self.direction_vector*self.target_projection_len,[0,0,1])
    ...
    def apply_ik_on_transition(self, spline):
        left_foot = self.skeleton.skeleton_model["joints"]["left_foot"] # "ball_l"
        right_foot = self.skeleton.skeleton_model["joints"]["right_foot"] # "ball_r"
    ...
    def set_action_constraint(self, action_name, keyframe_label, position, joint_name=None):
        ...
        self.action_constraint = UnityFrameConstraint((action_name, node), keyframe_label, joint_name, position, None)

    def transition_to_action(self, action="placeLeft"):
        if self.current_node[0] != "walk":
            return
        if self.node_type != NODE_TYPE_IDLE:
            self.node_queue.append((("walk", "endRightStance"), NODE_TYPE_END))
        for node_name, node_type in self.actions[action]["node_sequence"]:
            self.node_queue.append(((action, node_name), node_type))
        if self.node_type == NODE_TYPE_IDLE:
            self.node_queue.append((("walk", "idle"), NODE_TYPE_IDLE))
        else:
            self.node_queue.append((("walk", "beginLeftStance"), NODE_TYPE_START))
        self.transition_to_next_state_controlled()
